...
void init_socket_thing(void*) {
    ...
    g_tcpSocket = nn::socket::Accept(listenSocket, (struct sockaddr*)&serverAddr, &addrLen);
    nn::socket::Close(listenSocket);

    if (g_tcpSocket < 0) return;
...
bool TcpLogger::ShouldFlush() {
    return g_loggerInit.load(std::memory_order_acquire) && (g_tcpSocket != -1);
}
